...
- **Brand colors**: Indigo `#242E65` (primary — headers, buttons, chart colors, PPTX template) and halloween orange `#F05523` (accent — metric card left borders). Panel backgrounds: chat panel `#ffffff` (white), results panel `#f8f9fc` (light indigo tint, matches `config.toml` secondary). Panel divider: `1px solid #e0e2ef`. Applied via `.streamlit/config.toml` for native widgets and custom CSS in `app.py` for additional styling.
...
- **Chat interface**: 50/50 split layout (chat left, results right). Results panel hidden until first QBR generation. Both panels independently scrollable. Chat panel has white (`#ffffff`) background; results panel has light tinted (`#f8f9fc`) background with a `1px solid #e0e2ef` left border divider and `1.5rem` left padding. CSS uses `:nth-child(1)`/`:nth-child(2)` selectors on the outer `stHorizontalBlock` columns. Chat uses `st.chat_input` and `st.chat_message`. Intent parsing: regex pre-filter for common patterns, Claude Haiku fallback for ambiguous messages. Conversation history maintained in `st.session_state.chat_history`. Multi-turn follow-up questions for missing QBR fields (client, date range, MSP contact, employee count, industry).
