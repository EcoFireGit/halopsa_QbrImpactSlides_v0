...
- **`app.py`** — Streamlit UI. Single-page app with no sidebar (hidden via CSS). All credentials, AI settings, and API keys are managed through a `@st.dialog("Settings")` opened via a gear icon (`icon=":material/settings:"`) in the header. The dialog contains HaloPSA connection fields, AI recommendation toggle + sliders (num_recs, sample_size), and BEA API key. Auto-connects on launch when `.env` credentials exist (controlled by `auto_connect_attempted` flag). Main area uses collapsible `st.expander` sections for: (1a) Client & Date Range, (1b) Client Profile (employee count + avg hourly rate, persisted via `client_profiles.py`), Economic Context, MSP Contact Info, and Generate. After generation, a dashboard-style results view shows: download button, 4-column metrics row (Health Score badge, Same-Day Resolution, Avg First Response, Critical Resolution Time), Business Impact card (productivity hours lost + estimated dollar cost + risk statement), Risk Flags card (up to 5 flags with severity), and BEA panel. Metrics persist in `st.session_state.metrics_display` across reruns. Pipeline steps: 2a fetch tickets, 2b calculate metrics/health score, 2c compute business impact, 2d analyze risks, 2e generate AI recommendations (if enabled), 2f generate PPTX. No emojis anywhere in the UI.
...
- **Settings dialog**: Opened via `st.button(icon=":material/settings:")` in the page header. Contains HaloPSA credentials, AI toggle + sliders, and BEA API key. "Save" persists to session state; "Connect to HaloPSA" authenticates.
...
- **Dynamic recommendation slots**: `Master_QBR_Template.pptx` contains all 10 recommendation slots. At runtime, `_remove_unused_rec_slots()` in `generate_client_qbr.py` deletes shapes for slots N > num_recs (set in Settings dialog).
